## ============================================================ 
## PM2.5 and CMR — Hinge + Spline Models with AIC/BIC Comparison 
...
## ------------------------------------------------------------
## Define candidate hinge breakpoints
## ------------------------------------------------------------

lower_knots <- c(5.5, 6, 6.5)
upper_knots <- c(7.5, 8, 8.5)
...
## ------------------------------------------------------------
## Hinge model fitting function
## ------------------------------------------------------------

fit_hinge <- function(lower_knot, upper_knot) {
  fml <- as.formula(
    paste0(
      "CMR ~ PM2.5 +",
      "  I(pmax(PM2.5 - ", lower_knot, ", 0)^2) +",
      "  I(pmax(PM2.5 - ", upper_knot, ", 0)^2) +",
      "  civil_unemploy +",
      "  median_HH_inc +",
      "  femaleHH_ns_pct +",
      "  vacant_HHunit +",
...
## ------------------------------------------------------------
## Fit all hinge models
## ------------------------------------------------------------

models <- vector("list", nrow(knot_grid))

for (i in seq_len(nrow(knot_grid))) {
  models[[i]] <- fit_hinge(
    lower_knot = knot_grid$lower[i],
    upper_knot = knot_grid$upper[i]
  )
}

names(models) <- paste0("hinge_", knot_grid$lower, "_", knot_grid$upper)

## ------------------------------------------------------------
## Compute AIC/BIC for hinge models
## ------------------------------------------------------------
...
crit_df <- data.frame(
  model = names(models),
  lower_knot = knot_grid$lower,
  upper_knot = knot_grid$upper,
